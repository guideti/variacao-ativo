//
//  CoreDataSymbolsQueries.Storage.swift
//  app-ios
//
...
final class CoreDataSymbolsQueriesStorage {
...
extension CoreDataSymbolsQueriesStorage: SymbolsQueriesStorage {
    func fetchRecentsQueries(
        ...
        coreDataStorage.performBackgroundTask { context in
            do {
                let request: NSFetchRequest = SymbolQueryEntity.fetchRequest()
                request.sortDescriptors = [NSSortDescriptor(key: #keyPath(SymbolQueryEntity.createdAt),
                                                            ascending: false)]
    ...
    func saveRecentQuery(
        ...
        coreDataStorage.performBackgroundTask { [weak self] context in
            ...
            do {
                ...
                let entity = SymbolQueryEntity(symbolQuery: query, insertInto: context)
                try context.save()
...
// MARK: - Private
extension CoreDataSymbolsQueriesStorage {
    ...
    private func removeDuplicates(
        for query: SymbolQuery,
        in queries: inout [SymbolQueryEntity],
        inContext context: NSManagedObjectContext
    ) {
        queries
